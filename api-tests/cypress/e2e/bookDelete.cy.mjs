...
Then("the create response status code should be {int}", (statusCode) => {
...
When("I make a DELETE request to {string} with the id", (endpoint) => {
  cy.get("@authToken").then((token) => {
    cy.get("@response").its("body.id").invoke("toString").then((id) => {
        cy.deleteBook(id, endpoint, token).as("response");
    });
  }); 
});
